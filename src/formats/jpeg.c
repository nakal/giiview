...
int  open_jpeg(const char *filename, size_t owidth, size_t oheight) {
...
unsigned int getpixel_jpeg(size_t x, size_t y) {
	unsigned int val=0;
	unsigned char *p;

	if (error_occured) return 0x000000;

	if ((unsigned int)x>=cinfo.output_width ||
	    (unsigned int)y>=cinfo.output_height)
		return 0x0; else {
