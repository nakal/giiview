...
static FILE *bmpfile;
static size_t bitsperpixel, bytesinline, width, height;
...
int BMP_OpenFile(const char *filename, size_t owidth, size_t oheight)
{
	BitmapFileHeader bfh;
	BitmapInfo bi;
	int palettecolors,cnr,arraysize;

	(void)owidth; (void)oheight;
...
unsigned int BMP_GetPixel(size_t x,size_t y)
{
	unsigned char *offs;
	unsigned int color;
	unsigned int colorquad;

	if (x>=width || y>=height) return 0x000000;
